...
class Index:
    ...
    def _update_constituents(self, as_of_date: str) -> None:
        ...
        if self._selection_criteria:
            selected = self._selection_criteria.select(
                eligible,
                self._constituents  # Pass current for buffer rules
            )
        else:
            selected = eligible
    ...
    def get_timeseries(
        ...
        df = pd.DataFrame({
            "date": backtest.index_series.index,
            "value": backtest.index_series.values
        })
    ...
    def backtest(
        ...
        available_constituents = [constituent_lookup.get(t) or Constituent(ticker=t) for t in available_tickers]
    ...
    def validate(self) -> ValidationReport:
        ...
        if not self._identifier:
            report.add_error(
                field="identifier",
                message="Index identifier is not set"
            )
        
        # Validate constituents if available
        if self._constituents:
            constituent_report = self._validation_rules.validate_constituents(
                self._constituents
            )
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "selection_criteria": self._selection_criteria.to_dict() if self._selection_criteria else None,
            "weighting_method": self._weighting_method.to_dict() if self._weighting_method else None,
            "rebalancing_schedule": self._rebalancing_schedule.to_dict() if self._rebalancing_schedule else None,
            "validation_rules": self._validation_rules.to_dict() if self._validation_rules else None,
        }
