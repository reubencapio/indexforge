...
class WeightingMethod:
    ...
    @staticmethod
    def custom(
        weighting_fn: Callable[[List[Constituent]], Dict[str, float]]
    ) -> "WeightingMethod":
        """
    ...
    def _calculate_free_float_weights(self, constituents: List[Constituent]) -> Dict[str, float]:
        ...
        if total_ff_cap == 0:
            return self._calculate_equal_weights(constituents)
        return {
            c.ticker: (c.free_float_market_cap or c.market_cap) / total_ff_cap
