...
class SelectionCriteria:
    ...
    def _apply_buffer_rules(
        self,
        ranked: List[Tuple[Constituent, float]],
        ...
        # Create rank lookup
        rank_lookup = {c.ticker: i + 1 for i, (c, _) in enumerate(ranked)}
    ...
    def _apply_diversification(
        ...
        if len(final) < self.select_count:
            ...
            for constituent, _ in ranked:
                ...
                # Check constraints
                country_ok = not div.max_constituents_per_country or \
                    country_counts.get(country, 0) < div.max_constituents_per_country
                sector_ok = not div.max_constituents_per_sector or \
                    sector_counts.get(sector, 0) < div.max_constituents_per_sector
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            } if self.buffer_rules else None,
        }
...
class SelectionCriteriaBuilder:
    ...
    def apply_buffer_rules(
        self,
        add_threshold: Optional[int] = None,
        remove_threshold: Optional[int] = None
    ...
    def custom_ranking(
        self,
        ranking_fn: Callable[[Constituent], float]
    ) -> "SelectionCriteriaBuilder":
    ...
    def custom_filter(
        self,
        filter_fn: Callable[[Constituent], bool]
    ) -> "SelectionCriteriaBuilder":
