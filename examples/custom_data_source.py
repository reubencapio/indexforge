...
def main():
    ...
    data_provider = (DataProvider.builder()
        .add_source("custom", custom_connector)
        .set_default("custom")
        .build()
    )
    ...
    constituents = data_provider.get_constituent_data(
        universe.tickers,
        source="custom"
    )
    ...
    print(f"   📈 Retrieved {len(prices)} days of prices for {len(prices.columns.levels[0])} tickers")
